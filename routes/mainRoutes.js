...
router.post('/signup', async (req, res) => {
    try{
		...
		if (!req.body.userDob) throw "Email is not supplied.";
		...
		var regEx = /^[0-9a-zA-Z]+$/;
		if (!req.body.userName.match(regEx)) throw "Username must only contain alphanumeric characters!";
		if (!req.body.firstName.match(regEx)) throw "First Name must only contain alphanumeric characters!";
		if (!req.body.lastName.match(regEx)) throw "Last Name must only contain alphanumeric characters!";
		
		// ------------- checks if length of strings are enough ------------
		if (req.body.userName.length < 3) throw "Username must be at least 3 characters long.";
		if (req.body.firstName.length < 3) throw "First Name must be at least 3 characters long.";
		if (req.body.lastName.length < 3) throw "Last Name must be at least 3 characters long.";
		if (req.body.password.length < 8) throw "Password must be at least 8 characters long.";
		if (req.body.userEmail.length < 8) throw "Email must be at least 8 characters long.";
        
        // --------------------- call create user from user data ------------
        const user = await userData.createUser("profilepic", req.body.userName, req.body.password, req.body.firstName, req.body.lastName, req.body.userEmail, "dateofbirthda");
4
...
router.post('/home', async (req, res) => {
    ...
    try {
        ...
        templateObject.hasResults = (results.length > 0);
        templateObject.results = results.slice(0, 20);
        res.render('users/results', templateObject);
    } catch (e) {
        console.log(e);
